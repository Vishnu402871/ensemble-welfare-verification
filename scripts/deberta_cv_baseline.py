"""
DeBERTa-v3-base Supervised Baseline for IndiaWelfare-606
=========================================================
5-fold stratified cross-validation over all 521 gold pairs.
...
OUTPUTS:
  deberta_oof_predictions.csv   (per-pair held-out predictions, all seeds)
  deberta_cv_results.csv        (per-seed metrics + mean/SD summary)
...
print("\n" + "=" * 60)
print("DeBERTa-v3-base — 5-fold CV over 521 pairs (Mean ± SD, 3 seeds)")
print("=" * 60)
summary_rows = []
...
res.to_csv('deberta_cv_results.csv', index=False)
pd.DataFrame(summary_rows).to_csv('deberta_cv_summary.csv', index=False)
oof.to_csv('deberta_oof_predictions.csv', index=False)

print("\nSaved: deberta_cv_results.csv, deberta_cv_summary.csv,")
print("       deberta_oof_predictions.csv")
